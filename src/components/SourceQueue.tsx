...
function GitLabRow({ mr, onOpen, opened }: { mr: MergeRequest; onOpen: () => void; opened: boolean }) {
  return (
    <div role="button" tabIndex={0} aria-current={current(opened)} onClick={onOpen} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onOpen(); } }} className="cursor-pointer border-b border-[#30363d] px-4 py-3 hover:bg-[#161b22]">
...
export function SourceQueue({
  ...
  const sources = [
    ...(selection === "gitlab" ? [] : [{ name: "GitHub", rows: (githubLoading || githubError) && github.length === 0 ? undefined : github.length, loading: githubLoading, error: githubError, coverage: githubCoverage, staleSecs: null, retry: onRefreshGitHub }]),
  ...
  return <div className="space-y-3">
    ...
    {sources.map((source) => <div key={source.name}>
      ...
      {source.rows !== undefined && source.staleSecs !== null ? <p role="status" className="mt-1 text-xs text-[#d29922]">{source.name} is showing a saved list from {relativeSeconds(source.staleSecs)}.</p> : null}
    </div>)}
    <label className="block text-xs text-[#8b949e]">Search pull and merge requests
      <input value={query} onChange={(e) => setQuery(e.target.value)} className="mt-1 w-full rounded border border-[#30363d] bg-[#0d1117] px-3 py-2 text-sm text-[#e6edf3]" placeholder="Title, project, or number" />
    </label>
    <div className="rounded-md border border-[#30363d]">
      ...
      </p> : rows.map((row) => row.kind === "github"
        ? <PrRow key={prKey(row.value)} pr={row.value} onOpen={() => onOpen(prIdentity(row.value))} opened={selectedPr !== null && prKey(selectedPr) === prKey(row.value)} canWrite={canWriteGitHub} showSource />
        : <GitLabRow key={prKey(row.value)} mr={row.value} onOpen={() => onOpen(prIdentity(row.value))} opened={selectedPr !== null && prKey(selectedPr) === prKey(row.value)} />)}
    </div>
  </div>;
}
